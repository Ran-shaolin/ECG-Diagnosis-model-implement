...
class ECG_Net(nn.Module):
    def __init__(self, init_weights= False, num_classes=26, cfg = None):
        ...
        self.stage2 = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv1d(cfg[7], cfg[8], kernel_size=5, stride= 1, padding=2, bias=True),#原始输入的通道数为12
        ...
        self.stage3 = nn.Sequential(
            nn.Conv1d(cfg[8], cfg[9], kernel_size=5, stride= 1, padding=2, bias=True),#原始输入的通道数为12
            nn.BatchNorm1d(cfg[9]),
            nn.ReLU(),
            nn.Conv1d(cfg[9], cfg[10], kernel_size=5, stride= 1, padding=2, bias=True),#原始输入的通道数为12
            nn.BatchNorm1d(cfg[10]),
            nn.ReLU(),
            nn.Conv1d(cfg[10], cfg[11], kernel_size=5, stride= 1, padding=2, bias=True),#原始输入的通道数为12
        ...
        self.stage4 = nn.Sequential(
            nn.Conv1d(cfg[11], cfg[12], kernel_size=5, stride= 1, padding=2, bias=True),#原始输入的通道数为12
            nn.BatchNorm1d(cfg[12]),
            nn.ReLU(),
            nn.MaxPool1d(2, stride=2),
            nn.Conv1d(cfg[12], cfg[13], kernel_size=5, stride= 1, padding=2, bias=True),#原始输入的通道数为12
            nn.BatchNorm1d(cfg[13]),
            nn.ReLU(),
            nn.MaxPool1d(2, stride=2),
            nn.Conv1d(cfg[13], cfg[14], kernel_size=5, stride= 1, padding=2, bias=True),#原始输入的通道数为12
...
def updateBN(model,lamda):
    for m in model.modules():
        if isinstance(m, nn.BatchNorm1d):
            m.weight.grad.data.add_(lamda*torch.sign(m.weight.data))
            
def sparse_train(model,train_loader,val_loader,batch_size, num_epochs,loss_fun,optimizer,lamda):
    train_losses = []   
    valid_losses = []    
    avg_train_losses = []    
    avg_valid_losses = [] 
    ...
    for epoch in tqdm_notebook(range(1, num_epochs + 1)):        
        ...
        train_losses = []
        valid_losses = []
        torch.save({'epoch': epoch + 1, 'state_dict': model.state_dict()},'./ECG/ECG_sparse_model' + str(epoch) + '.pth.tar')
    return  model, avg_train_losses, avg_valid_losses

def fine_tune(model,train_loader,val_loader,batch_size, num_epochs,loss_fun,optimizer):
    train_losses = []   
    valid_losses = []    
    avg_train_losses = []    
    avg_valid_losses = [] 
    ...
    for epoch in tqdm_notebook(range(1, num_epochs + 1)):
        ...
        for batch_idx, (data, label) in tqdm_notebook(enumerate(train_loader)):
            ...
            train_acc_num = calculate_accuracy(output, label)
            train_running_acc += train_acc_num
        ...
        torch.save({'epoch': epoch + 1, 'state_dict': model.state_dict()},'./ECG/Pruned_model/ECG_model' + str(epoch) + '.pth.tar')
    return  model, avg_train_losses, avg_valid_losses